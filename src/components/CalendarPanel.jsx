...
export default function CalendarSidebar() {
  const { calendarOpen, toggleCalendar, openLogModal } = useUIStore();
  ...
  return (
    <div style={{
      width: calendarOpen ? 280 : 56,
      minWidth: calendarOpen ? 280 : 56,
      ...
      {!calendarOpen && (
        <div style={{
          ...
          padding: '14px 0', gap: 10, width: 56,
        }}>
          {/* Toggle button */}
          <button
          ...
          {/* Today's date chip */}
          <div style={{
            width: 36, height: 40, borderRadius: 'var(--r-sm)',
            background: 'var(--accent)',
            display: 'flex', flexDirection: 'column',
            alignItems: 'center', justifyContent: 'center',
            boxShadow: 'var(--shadow-accent)',
          }}>
            <span style={{ fontSize: 8, fontWeight: 700, color: 'rgba(255,255,255,0.7)', textTransform: 'uppercase', lineHeight: 1 }}>
              {format(today, 'MMM')}
            </span>
            <span style={{ fontSize: 16, fontWeight: 800, color: '#fff', lineHeight: 1, marginTop: 1, fontFamily: "'Bricolage Grotesque', sans-serif" }}>
              {format(today, 'd')}
            </span>
          </div>

          {/* Divider */}
          <div style={{ width: 20, height: 1, background: 'var(--border)', margin: '2px 0' }} />

          {/* Upcoming deadline dots */}
          {upcoming.slice(0, 4).map((item) => {
            ...
            return (
              <button
                key={item.id}
                onClick={toggleCalendar}
                title={`${item.name} — ${format(d, 'MMM d')}`}
                style={{
                  width: 36, height: 36, borderRadius: 'var(--r-sm)',
                  background: isUrgent ? 'var(--warm-softer)' : 'var(--surface)',
              ...
              >
                ...
                <div style={{
                  ...
                  background: item.sourceType === 'goal' ? 'var(--accent)' : 'var(--warm)',
                }} />
              </button>
            );
          })}
